...
def main_form_copy(root, lang):
    ...
    return {
        ...
        'attachmentsHeading': form.select_one('.file-upload-input').find_previous('h4').get_text(strip=True),
        'fileChoose': form.select_one('.file-upload-btn .file-msg').get_text(strip=True),
        'fileBrowse': form.select_one('.file-upload-btn .btn-txt').get_text(strip=True),
    }
...
def order_catalog_version(data):
    products = client_products(data, 'cs')
    return hashlib.sha256(json.dumps(products, ensure_ascii=False, sort_keys=True).encode()).hexdigest()


def build_order_catalog(data, translations, email_translations, output_root):
    """Keep the mail handler's authoritative products/prices in a non-output PHP file."""
    products = client_products(data, 'cs')
    version = order_catalog_version(data)
    if set(email_translations) != set(data['languages']):
        raise ValueError('Incomplete order email languages')
    for lang, copy in email_translations.items():
        if set(copy) != set(email_translations['cs']) or any(not isinstance(value, str) or not value.strip() for value in copy.values()):
            raise ValueError(f'Incomplete order email translation: {lang}')
    locales = {lang: {'products': client_products(data, lang), 'text': translations[lang],
                      'email': email_translations[lang]} for lang in data['languages']}
    catalog = json.dumps({'products': products, 'languages': data['languages'], 'text': translations['cs'],
                          'version': version, 'locales': locales}, ensure_ascii=False, separators=(',', ':'))
    literal = catalog.replace('\\', '\\\\').replace("'", "\\'")
    target = output_root / 'backend/order_catalog.php'
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("<?php\n// Generated by build_shop.py; do not edit.\nreturn json_decode('" + literal + "', true, 32, JSON_THROW_ON_ERROR);\n", encoding='utf-8')
    return version


def build_shop(root=ROOT, languages=None, output_root=None, include_backend=True):
    ...
    if include_backend:
        email_translations = json.loads((root / 'shop/order-email-translations.json').read_text(encoding='utf-8'))
        catalog_version = build_order_catalog(data, translations, email_translations, output_root)
    else:
        catalog_version = order_catalog_version(data)
    ...
    for lang in languages:
        ...
        config = json.dumps({'products': client_products(data, lang), 'currency': data['currency'], 'catalogVersion': catalog_version,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frontend-only', action='store_true', help='Render pages without generating backend/order_catalog.php.')
    args = parser.parse_args()
    build_shop(languages=args.language, include_backend=not args.frontend_only)
